  ...
  async getClassifyByQuery(
    ...
    if (queryString && searchFields) {
      ...
      const conditions = fields
        .map((field) => {
          if (field === 'fruit') return 'LOWER(fruit.fruit_name) LIKE LOWER(:query)';
          if (field === 'areaClassify') return 'LOWER(areaClassify.area_code) LIKE LOWER(:query)';
          return `LOWER(fc.${field}) LIKE LOWER(:query)`;
        })
        .join(' OR ');
