...
export class RaspberryController {
  ...
  @Post('/generate-token')
  async getRaspberryAccessToken(@Body('device_code') device_code: string) {
    if (!device_code) {
  ...
  @Post('/update-config')
  @UseInterceptors(
    FileInterceptor('raspberry_model', {
      storage: diskStorage({
      ...
      fileFilter: (req, file, cb) => {
        if (!file.originalname.endsWith('.tflite') && !file.originalname.endsWith('.pkl')) {
          return cb(new BadRequestException('Chỉ được phép upload file .tflite hoặc .pkl'), false);
        }
        cb(null, true);
      },
    }),
  ...
  async updateRaspberryConfig(
    ...
    const { upsertConfig, configData } = await this.raspberryService.updateRaspberryConfig(
      data,
      modelPath,
    );
